...
class PostgresBenchmark(DatabaseBenchmark):
    ...
    def _execute_fetchall(self, query: str, params: tuple) -> int:
        """helper to execute query and return row count by fetching all results"""
    ...
    def q2_variant_by_position(self, chromosome: str, position: int) -> int:
        return self._execute_fetchall(
            "SELECT * FROM variants WHERE chrom = %s AND pos = %s",
            (chromosome, position),
        )
    ...
    def q4_variants_in_gene_all(self, gene: str) -> int:
        return self._execute_fetchall(
            """
            SELECT v.*, a.gene_symbol, a.consequence, a.impact 
            FROM variants v
    ...
    def q10_coding_variants(self, consequences: List[str], gene: str = None) -> int:
        sql = """
            SELECT DISTINCT v.* FROM variants v
            JOIN annotations a ON v.id = a.variant_id
